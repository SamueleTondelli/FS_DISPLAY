...
void initializeDataPowerUnit(DataPowerUnit* dataPowerUnit) {
	initializeData(&(dataPowerUnit->rpm), EMPTY_VALUE, RPM_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->gear), EMPTY_VALUE, GEAR_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->oilPressure), EMPTY_VALUE, OIL_PRESSURE_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->oilTemperature), EMPTY_VALUE, OIL_TEMPERATURE_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->coolantTemperatureCV), EMPTY_VALUE, COOLANT_TEMPERATURE_CV_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->vehicleSpeed), EMPTY_VALUE, VEHICLE_SPEED_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->ignitionTiming), EMPTY_VALUE, IGNITION_TIMING_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->fuelPressure), EMPTY_VALUE, FUEL_PRESSURE_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->exhaustLambda), EMPTY_VALUE, EXHAUST_LABDA_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->lambdaTarget), EMPTY_VALUE, LABDA_TARGER_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->ambientTemperature), EMPTY_VALUE, AMBIENT_TEMPERATURE_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->airTemperature), EMPTY_VALUE, AIR_TEMPERATURE_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->map), EMPTY_VALUE, MAP_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->engineChargeTemperature), EMPTY_VALUE, ENGINE_CHARGE_TEMPERATURE_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->inletMainfoldPressure), EMPTY_VALUE, INLET_MAINFOLD_PRESSURE_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->bevelGearTemperature), EMPTY_VALUE, BEVER_GEAR_TEMPERATURE_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->injectedFuelFlow), EMPTY_VALUE, INJECTED_FUEL_FLOW_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->injectedFuelVolume), EMPTY_VALUE, INJECTED_FUEL_VOLUME_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->fuelClosedLoopControlBank1), EMPTY_VALUE, FUEL_CLOSED_LOOP_CONTROL_BANK1_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->fuelUsed), EMPTY_VALUE, FUEL_USED_CONVERSION_FACTOR);
	initializeData(&(dataPowerUnit->fuelInjectorPrimaryDifferentialPressure), EMPTY_VALUE, FUEL_INJECTOR_PRIMARY_DIFFERENTIAL_PRESSURE_CONVERSION_FACTOR);
